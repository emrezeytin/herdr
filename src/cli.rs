...
const TERMINAL_SESSION_OBSERVE_USAGE: &str =
    "usage: sessionr terminal session observe <target> [--cols N] [--rows N]";
const TERMINAL_SESSION_CONTROL_USAGE: &str =
    "usage: sessionr terminal session control <target> [--takeover] [--cols N] [--rows N]";
pub(crate) const AGENT_HELP_FOOTER: &str = concat!(
    "Are you an AI? Use these resources ONLY IF your task specifically asks you to:\n",
    "  Help a human understand or set up sessionr for the first time:\n",
    "    https://herdr.dev/agent-guide.md\n",
    "  Debug or investigate a problem with sessionr:\n",
    "    https://herdr.dev/llms.txt\n",
    "  Control sessionr panes, agents, or workspaces:\n",
    "    SKIP if a sessionr skill is already in your context. Otherwise run: sessionr --skill",
);
...
fn channel_set(args: &[String]) -> std::io::Result<i32> {
    let Some(channel) = parse_channel_set_arg(args) else {
        eprintln!("usage: sessionr channel set <stable|preview>");
        return Ok(2);
    };
    ...
    println!(
        "sessionr update channel set to {channel} in {}.",
        path.display()
    );
    ...
    if let Err(err) = crate::update::self_update(crate::update::SelfUpdateOptions::default()) {
        ...
        eprintln!("Run `sessionr update` to retry.");
        return Ok(1);
    }
...
fn print_channel_help() {
    eprintln!("sessionr channel commands:");
    eprintln!("  sessionr channel show                  print the configured update channel");
    eprintln!("  sessionr channel set <stable|preview>  choose the update channel");
}
...
fn config_check(args: &[String]) -> std::io::Result<i32> {
    match args {
        ...
        [flag] if matches!(flag.as_str(), "help" | "--help" | "-h") => {
            eprintln!("usage: sessionr config check");
            return Ok(0);
        }
        _ => {
            eprintln!("usage: sessionr config check");
...
fn config_reset_keys(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: sessionr config reset-keys");
        return Ok(2);
    }
    ...
    println!("Built-in v2 keybindings will apply after sessionr restarts or reloads config.");
    println!("If a sessionr server is running, run `sessionr server reload-config` to apply this now.");
    println!(
        "To restore: cp {} {}",
        backup_path.display(),
        path.display()
...
fn terminal_attach(args: &[String]) -> std::io::Result<i32> {
    let (terminal_id, takeover) = match parse_attach_target(
        args,
        "usage: sessionr terminal attach <terminal_id> [--takeover]",
    ) {
        Ok(parsed) => parsed,
        Err(code) => return Ok(code),
    };
...
fn terminal_title(args: &[String]) -> std::io::Result<i32> {
    match args.first().map(|arg| arg.as_str()) {
        Some("set") => {
            if args.len() != 2 {
                eprintln!("usage: sessionr terminal title set <title>");
                return Ok(2);
            }
        ...
        Some("clear") => {
            if args.len() != 1 {
                eprintln!("usage: sessionr terminal title clear");
                return Ok(2);
            }
            print_response(&send_request(&Request {
        ...
        Some("help" | "--help" | "-h") => {
            eprintln!("usage: sessionr terminal title set <title>");
            eprintln!("       sessionr terminal title clear");
            Ok(0)
        }
        _ => {
            eprintln!("usage: sessionr terminal title set <title>");
...
fn print_config_help() {
    eprintln!("sessionr config commands:");
    eprintln!("  sessionr config check  validate config.toml and print diagnostics");
    eprintln!("  sessionr config reset-keys  back up config.toml and remove custom keybindings");
}

fn print_terminal_help() {
    eprintln!("sessionr terminal commands:");
    eprintln!("  sessionr terminal attach <terminal_id> [--takeover]");
    eprintln!("  sessionr terminal session control <target> [--takeover] [--cols N] [--rows N]");
    eprintln!("  sessionr terminal session observe <target> [--cols N] [--rows N]");
    eprintln!("  sessionr terminal title set <title>");
    eprintln!("  sessionr terminal title clear");
